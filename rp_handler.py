...
• Starts one Fish-Speech server (port 3000) with the 1.5 checkpoint.
• Accepts OpenAI-compatible JSON: {"input": "...", "voice": "...", "response_format": "..."}.
• Returns WAV as one base-64 string or streams OGG chunks (base-64).
"""

import os, time, asyncio, base64, subprocess, traceback
import runpod, aiohttp, httpx

# ───────────── Launch Fish-Speech once ─────────────
...
async def handler(job):
    inp   = job.get("input", {})
    text  = inp.get("input", "")             # OpenAI field
    voice = inp.get("voice", "default")
    fmt   = inp.get("response_format", "wav").lower()
    _     = inp.get("model")                 # accepted but ignored
